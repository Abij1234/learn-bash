#!/usr/bin/env bash
printf "\033[1;44mIT IS USED TO UPDATE leran-bash TO GET MORE CHAPTERS WHEN ADDED \033[0m\n"
echo
printf "\033[1;36mupdate>type UPDATE to update \033[0m\n"
echo
printf "\033[1;36mexit>type EXIT to exit \033[0m\n"
echo
while true; do
printf "\033[1;34mENTER YOUR OPTION HEAR==> \033[0m"
read opt

if [[ $opt == 'update' || $opt == 'UPDATE' ]]; then
echo
printf "\033[1;36mPLEASE WAIT \033[0m\n"
echo
printf "LORDING"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#"
sleep 0.1
printf "#100%%"
echo
echo
git pull
echo
echo "its finished run learn bash again then you can see next chapters" | pv -qL 100 | lolcat
break

elif [[ $opt == 'exit' || $opt == 'EXIT' ]]; then
echo
printf "\033[1;31mYOU ARE EXITED \033[0m\n"
exit 1

else
echo
printf "\033[1;41m[!] PLEASE SELECT A VALID OPTION \033[0m\n"
fi
done

